<template>
	...
	<section class="h-screen image-section">
		<div class="flex flex-col content-end h-full bg-gradient-to-t from-background_dark to-transparent">
			<div class="mt-28"></div>
			<div
			...
			>
				<div class="flex flex-col px-10 space-y-5 ">
					...
					<div class="flex flex-row flex-wrap max-w-md text-white">
						...
						<p class="px-1 mr-5 border-2 rounded border-background_light text-primary-100">{{ age }}</p>
							<p class="mr-5 italic">{{length}}</p>
							<div class="mr-5 border-l-2 border-background_light"></div>
					...
					<div class="flex flex-col max-w-md text-white">
						...
						<button @click="$router.push('/watch/' + id)" id="watch_button" class="p-2 bg-gray-900 bg-opacity-50 rounded-lg shadow-lg w-36 hover:bg-primary-200 focus:bg-primary-200 hover:bg-opacity-30 focus:bg-opacity-30" tabindex="0">Watch Now</button>
			...
			<div id="movies-row" class="z-0 flex flex-row flex-wrap transition-transform md:content-center p-7" @mouseenter="moveUp" @mouseleave="moveDown" @focusin="moveUp" @focusout="moveDown">
				<div tabindex="0" id="poster" class="mb-5 mr-5 transition-all transform bg-center bg-no-repeat bg-cover shadow-sm cursor-pointer group focus:border-4 hover:border-4 hover:scale-105 aspect-16/9 w-96 rounded-xl" v-for="movie in otherMovies.slice(0, 3)" :style="'background-image: url(\'' + movie['poster'] +'\')'">
				...
				<div @click="$router.push('/library')" tabindex="0" id="poster" class="relative mb-5 mr-5 transition-all transform bg-center bg-no-repeat bg-cover shadow-sm cursor-pointer group focus:border-4 hover:border-4 hover:scale-105 aspect-16/9 w-96 rounded-xl bg-background_dark">
...
const bannerMovie = {
	...
	rating: "101",
	original: true,
	description: "Captain Pineapple is a comedy-drama film directed by John Hughes, written by John Hughes and starring Tom Holland, Julia Louis-Dreyfus, and Robert Pattinson. The film stars Tom Holland in a story about a group of friends who are forced to deal with the consequences of their actions in the world of pineapple. The film is set in the fictional country of Pineapple, which is located in the Caribbean. The film is based on the pineapple story of the same name by John Hughes. The film is the first film to use the pineapple as a subject in a comedy.",
}
