    ...
    <div class="backdrop-blur-md h-full w-full px-10">
      ...
      <FlexRow>
        <Poster
          ...
          @mouseenter="bg = `https://api.bergflix.de${serie.attributes!.background_image!.data?.attributes?.formats.large?.url || serie.attributes!.background_image!.data?.attributes?.formats.medium.url}`"
          @mouseleave="bg = ''"
        />
      </FlexRow>
    </div>
...
const series = useStrapi<SerieEntity[]>(
  ...
  ],
  {
    onSuccess: (data) => {
      data.forEach((serie) => {
        preloadImage(
          serie.attributes?.background_image?.data?.attributes?.formats.medium.url
        );
      });
    },
  }
