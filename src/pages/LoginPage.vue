<template>
  <div id="container" class="flex items-center justify-center w-full h-screen">
    ...
    >
      ...
      <p class="text-green-300" v-if="loggedIn">
        Logged in successfuly! You will be redirected any second...
      </p>
      ...
      <form class="flex flex-col items-center justify-center" @submit.prevent="login">
        ...
        <input
          ...
          required
        />
        <Button type="link" :to="refer ? `register?ref=${refer}` : 'register'"
...
const login = () => {
  strapi
    ...
    .then((res: StrapiAuthenticationResponse) => {
      loggedIn.value = true;
