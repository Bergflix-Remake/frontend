  ...
  <article v-if='!invalidId' class='p-5 flex flex-col w-full min-h-screen space-y-2 mt-20'>
    <div class='flex flex-col xl:flex-row xl:space-x-2'>
      ...
      <div class='bg-clean-dark-600 w-full aspect-video md:rounded-l-lg  overflow-hidden'>
        <Player  v-if='movie.isSuccess && !playbackFinished' :key='url!'  :vid='url!' :page-id='id' :outro_time="movie.data.attributes?.outro_start || undefined" @finished="playNext()" />
        <div v-else class='w-full h-full justify-center items-center flex'>
          <div v-if="playbackFinished" class="flex flex-col justify-center items-center">
            <h2 class="text-2xl font-bold">Danke fürs Ansehen!</h2>
            <p v-if="series?.videos?.data.length" class="text-delorean-500 font-thin">Die nächste Folge wird in ein paar momenten Abgespielt...</p>
            <p v-else class="text-delorean-500 font-thin">Das könnte dich auch interessieren:</p>
            <Poster v-if="recommendations.isLoading" loading />
            <Poster v-else-if="recommendations.isSuccess" :image="recommendation?.attributes?.thumbnail?.data?.attributes?.url" @click="router.push(`/watch/${recommendation?.id!}`)"/>
            <Button class="m-2" @click="router.push('/')">Home</Button>
          </div>
          <Loader v-else />
        </div>
      </div>
...
const consoleStyles = 'background-color: lime; color: white; padding: 2px 4px; border-radius: 4px; font-weight: bold; font-family: sans-serif; margin: 10px 0;';
...
const recommendation = computed(() => recommendations.data?.[Math.floor(Math.random() * recommendations.data?.length)]);
...
const playNext = () => {
  ...
  if (series.value?.videos?.data?.length) {
    const next = series.value.videos.data.find(video => video.attributes?.episode === movie.data?.attributes?.episode! + 1)
    if (next) {
      console.debug('%cWatchpage', consoleStyles, 'Playing next episode', next.id)
