<template>
  <details
    v-if="error"
    class="text-red-500 m-5 border-red-500 border-2 p-3 cursor-pointer relative rounded-xl"
  >
    <summary>
      Es ist ein Fehler aufgetreten!
      <ExclamationCircleIcon class="w-4 h-4 inline mb-1" />
    </summary>
    <span class="font-mono">{{ error.error.status }}</span>
    {{ error.error.name }} <br />
    <span
      v-if="error.error.details && error.error.details.errors"
      class="text-xs"
    >
    ...
    <p class="max-w-md">
      Wenn du uns helfen möchtest, melde den Fehler bitte auf unserem
      <Href to="https://bflx.de/discord">Discord</Href>
    </p>
    <!-- Copy button -->
    <button
