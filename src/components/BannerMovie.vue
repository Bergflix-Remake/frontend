...
>
    <div class="flex flex-col px-10 space-y-5 ">
        <img
            v-if="banner_movie.series && banner_movie.series.title_image"
            :src="banner_movie.series.title_image"
            :alt="banner_movie.title"
            class="w-96 mb-7"
        />
        <h1 v-else-if="banner_movie.series && banner_movie.series.title" class="text-5xl font-black text-white">{{ banner_movie.series.title }}</h1>
        ...
        <div class="flex flex-col max-w-md text-white">
            ...
            <div class="flex flex-row space-x-5">
                <button @click="$router.push('/watch/' + banner_movie.id )" id="watch_button" class="flex flex-row p-2 bg-gray-900 bg-opacity-50 rounded shadow-lg min-w-min hover:bg-primary-200 focus:bg-primary-200 hover:bg-opacity-30 focus:bg-opacity-30" tabindex="0"><PlayIcon class="w-5 mr-2 text-white"/>Watch Now</button>
                <button @click="$router.push('/media/' + banner_movie.id )" id="watch_button" class="flex flex-row p-2 bg-gray-900 bg-opacity-50 rounded shadow-lg min-w-min hover:bg-primary-200 focus:bg-primary-200 hover:bg-opacity-30 focus:bg-opacity-30" tabindex="0"><CursorClickIcon class="w-5 mr-2 text-white"/>More Info</button>
            </div>
...
import { get_movie, get_series, error_movie, get_banner_movie } from '../api_handler'
...
const banner_movie: Movie = await get_banner_movie()
...
.catch(err => {
    console.log(err);
    return error_movie("Could not get movie.")
});
