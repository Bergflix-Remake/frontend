...
function useStrapiQuery<T>({ queryKey: [contentType, params] }: any) {
  return strapi.find<T>(contentType, params).then((res) => {
    return res.data as T;
  });
}

function useStrapiQueryOne<T>({ queryKey: [contentType, id, params] }: any) {
  return strapi.findOne<T>(contentType, id, params || {}).then((res) => {
    return res.data;
  });
}
...
export const useStrapiUpdateMutation = <
  TData,
  TVariables = Partial<TData>,
  TContext = Partial<TData>,
>(
  opts?: UseMutationOptions<
    StrapiResponse<TData>,
    TError,
    StrapiUpdateData<TVariables>,
    TContext
  >,
) => {
  return useMutation(
    async (data: StrapiUpdateData<TVariables>) =>
      await strapi.update<TData>(
        data.contentType,
        data.id,
        data.data,
        data.params,
...
export const useResendEmailMutation = () =>
  useMutation<void, TError, StrapiEmailConfirmationData, unknown>(
    async (data: StrapiEmailConfirmationData) =>
      await strapi.sendEmailConfirmation(data),
  );
...
export function useStrapiSendEmailConfirmation(
  ...
  options?: Omit<
    UseQueryOptions<void, TError, void, [StrapiEmailConfirmationData]>,
    'queryKey' | 'queryFn'
  >,
  ...
  return reactive(
    useQuery(
      queryKey,
      async ({ queryKey }) => await strapi.sendEmailConfirmation(queryKey[0]),
      options,
    ),
  );
}
...
export function getUser(
  ...
  opts?: Omit<
    ...
    'queryFn' | 'queryKey'
  >,
): UnwrapNestedRefs<UseQueryReturnType<UsersPermissionsUser, TError>> {
  return reactive(
    useQuery<UsersPermissionsUser, TError>(
      ['user', props],
      async () => {
        await strapi.fetchUser();
        const user = await strapi.request<UsersPermissionsUser>(
          'GET',
          '/users/me',
          { params: props },
        );
